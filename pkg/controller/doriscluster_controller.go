...
func (r *DorisClusterReconciler) watchConfigMapBuilder(builder *ctrl.Builder) *ctrl.Builder {
	...
	p := predicate.Funcs{
		UpdateFunc: func(u event.UpdateEvent) bool {
			...
			if !ok {
				klog.Errorf(
					"Failed to cast ObjectOld to ConfigMap %s: objectKind=%v, objectName=%s, objectNamespace=%s",
					"ObjectOld",
			...
			if !ok {
				klog.Errorf(
					"Failed to cast ObjectNew to ConfigMap %s: objectKind=%v, objectName=%s, objectNamespace=%s",
					"ObjectOld",
					u.ObjectOld.GetObjectKind().GroupVersionKind(),
					u.ObjectOld.GetName(),
					u.ObjectOld.GetNamespace(),
			...
			return !set.CompareMaps(oldConfigMap.Data, newConfigMap.Data)
		},
	}
...
func (r DorisClusterReconciler) Obj2ConfigMap(u event.UpdateEvent) bool {
	return false
}
