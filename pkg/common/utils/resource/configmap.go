...
func getDorisConfigInfo(dcr *dorisv1.DorisCluster, componentType dorisv1.ComponentType) string {
	...
	var cm dorisv1.ConfigMapInfo
	switch componentType {
	case dorisv1.Component_FE:
		cm = dcr.Spec.FeSpec.ConfigMapInfo
	case dorisv1.Component_BE:
		cm = dcr.Spec.BeSpec.ConfigMapInfo
	case dorisv1.Component_CN:
		cm = dcr.Spec.CnSpec.ConfigMapInfo
	case dorisv1.Component_Broker:
		cm = dcr.Spec.BrokerSpec.ConfigMapInfo
	default:
		klog.Infof("getDorisConfigInfo: the componentType: %s have not default ResolveKey", componentType)
	}

	maps := GetMountConfigMapInfo(cm)
	for i := range maps {
...
func GetDorisCoreConfigMaps(dcr *dorisv1.DorisCluster) (dorisCoreConfigMaps map[dorisv1.ComponentType]string) {
